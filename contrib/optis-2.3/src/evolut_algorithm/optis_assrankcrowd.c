...
void assign_rank_and_crowding_distance (population *new_pop, int popsize, int nobj)
{
    int flag;
    int i;
    int end;
    ...
    do
    {
        ...
        do
        {
            ...
            do
            {
                ...
                flag = check_dominance (&(new_pop->ind[temp1->index]), &(new_pop->ind[temp2->index]), nobj);
                if (flag == 1)
                {
        ...
        while (temp2 != NULL);
        assign_crowding_distance_list (new_pop, cur->child, front_size, nobj);
        temp2 = cur->child;
        do
        {
            temp2 = del (temp2);
