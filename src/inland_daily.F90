...
! This is the 0D version of the daily subroutine (formerly known as dailyF)
! Copied on its entirety from Hewlley Imbuzeiro's 0D version. -fzm
subroutine daily (test, dimforc)
      use inland_parameters, only: grav, rair, dtime
      use inland_control, only: iyear, imonth, iday
      use inland_comatm, only: tmin, precip, psurf, qd
      use inland_comforc, only: xta, xprec, xqa
      use inland_comsum, only: td
      use inland_comsatp
...
! Calcul of the actual td, precip, tmin, and qd
      it0 = dimforc - test
      tmin(1) = 373.16
      td(1) = 0.
      ed = 0.
      precip(1) = 0.
      do it = it0+1, it0+niter
         td(1) = td(1) + (xta(1,it) + 273.16)/float(niter)
         precip(1) = precip(1) + xprec(1,it)
         tmin(1) = min(tmin(1) , xta(1,it) + 273.16)
         ed = ed + xqa(1,it) /float(niter)
      end do
      ed = ed / 100.
...
! Calcul of psurf(specific for the Flona km 83 site)
      psurf(1) = 99208.7369088147 - 21.7473169490388 * (td(1) - 273.16)
 
! convert from relative humidity to specific humidity at
! daily mean temperature
      qd(1) = ed * qsat(esat(td(1)), psurf(1))

#else /* SINGLE_POINT_MODEL */
! This is the ordinary 'daily' subroutine for the 2D weather generator.
...
!      iyrlast, nrun are not used for now
subroutine daily (seed, seed2,seed3,seed4, jdaily, iyrlast, nrun)
! ---------------------------------------------------------------------
! 
! overview
! 
...
! local variables
      ...
      integer :: iyrlast, nrun ! these arguments are not used for now
...
! calculate monthly-average probability of rainy day 
            pwet = max (dble(1.), xinwet(i,imonth)) / ndaypm(imonth)
...
! of precipitations (negative values, or too few wet days in a rainy month)
               xinprec(i, imonth) = max(dble(0.), xinprec(i, imonth))
               xinwet(i, imonth) = max(dble(1.), xinwet(i, imonth))
9000           continue
...
! anomalies 
               if ((precipdaysum(i) .eq. 0).AND.(xinprec(i,imonth) .gt. 0)) then
                  ...
                  if (precipday(i,j).gt.360) then
                     if (xinwet(i,imonth) .lt. ndaypm(imonth)) then
                        ...
                        print *, 'WARNING: goto 9000a', i,             &
                                 int(xinwet(i,imonth)), iwetdaysum(i), &
                                 int(precipday(i,j))
                          if(precipday(i,j).le.0. .or. &
                             precipday(i,j).ge.100000.) then
                             print *,"xinprec(i,month):",xinprec(i,imonth)," xinwet():", xinwet(i,imonth)," i:", i, " month:",imonth
...
240            continue
               ...
               dif = precipdaysum(i) - xinprec(i,imonth) * float(ndaypm(imonth))
...
! mean daily mean temperature (K) 
            tdm = xint(i,it1w) + dt * (xint(i,it2w) - xint(i,it1w)) + 273.16
